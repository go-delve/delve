					...
					default:
						...
						for _, t := range tracepoints {
							...
							for _, p := range t.InputParams {
								...
								params.WriteString(ebpfTraceParamString(p))
							}
...
// ebpfTraceParamString formats a traced parameter for display. For simple
// types (int, bool, string) it returns the Value string directly. For complex
// types (structs, arrays) it recursively formats Children into "{f1 f2 ...}".
func ebpfTraceParamString(p api.Variable) string {
	if p.Unreadable != "" {
		return fmt.Sprintf("<error: %s>", p.Unreadable)
	}
	...
	if len(p.Children) > 0 {
		...
		for _, c := range p.Children {
			parts = append(parts, ebpfTraceParamString(c))
		}
