...
func wait(pid, tgid, options int) (int, *sys.WaitStatus, error) {
	...
	if (pid != tgid) || (options != 0) {
		...
		// if we call wait4/waitpid on a thread that is the leader of its group, with options == 0, while ptracing and the thread leader has exited leaving zombies of its own then waitpid hangs forever
		// this is apparently intended behaviour in the linux kernel because it's just so convenient
		// therefore we call wait4 in a loop with WNOHANG, sleeping a while between calls and exiting when either wait4 succeeds or we find out that the thread has become a zombie.
		// References:
