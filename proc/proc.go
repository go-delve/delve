...
func (dbp *Process) SetBreakpoint(addr uint64) (*Breakpoint, error) {
	...
	return dbp.setBreakpoint(dbp.CurrentThread.ID, addr, NormalBreakpoint)
}
...
func (dbp *Process) Next() (err error) {
	...
	for i := range dbp.Breakpoints {
		if dbp.Breakpoints[i].Kind != NormalBreakpoint {
...
func (dbp *Process) stepToPC(pc uint64) error {
	if _, err := dbp.SetTempBreakpoint(pc, NextBreakpoint, sameGoroutineCondition(dbp.SelectedGoroutine)); err != nil {
		return err
	}
	return dbp.Continue()
}
...
func (dbp *Process) ClearTempBreakpoints() error {
	...
	for i := range dbp.Threads {
		if dbp.Threads[i].CurrentBreakpoint != nil && dbp.Threads[i].CurrentBreakpoint.Kind != NormalBreakpoint {
