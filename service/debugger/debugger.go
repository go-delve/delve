...
func (d *Debugger) GuessSubstitutePath(clientMod2Dir map[string]string) map[string]string {
	serverMod2Dir := map[string]string{}
	serverMod2DirCandidate := map[string]map[string]int{}

	for mod := range clientMod2Dir {
	...
	for tgt.Next() {
		...
		for i := range bi.Functions {
			...
			if fn.CompileUnitName() != "" && strings.ReplaceAll(fn.CompileUnitName(), "\\", "/") != fnpkg {
				// inlined
				continue
			}
			fnmod := ""
			...
			file, _ := bi.EntryLineForFunc(fn)
			if file == "" || file == "<autogenerated>" {
				continue
			}
			logger.Debugf("considering %s pkg:%s compile unit:%s file:%s", fn.Name, fnpkg, fn.CompileUnitName(), file)
	...
	for mod, clientDir := range clientMod2Dir {
		if serverMod2Dir[mod] != "" {
