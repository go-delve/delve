...
func (d *Debugger) checkGoVersion() error {
	...
	producer := d.target.Sel.BinInfo().Producer()
	if producer == "" {
		return nil
	}
...
func (d *Debugger) TargetGoVersion() string {
	...
	return d.target.Sel.BinInfo().Producer()
}
...
func (d *Debugger) LastModified() time.Time {
	d.targetMutex.Lock()
	defer d.targetMutex.Unlock()
	return d.target.Sel.BinInfo().LastModified()
}
...
func (d *Debugger) Restart(rerecord bool, pos string, resetArgs bool, newArgs []string, newRedirects [3]string, rebuild bool) ([]api.DiscardedBreakpoint, error) {
	...
	d.target = proc.NewGroupWithBreakpoints(p, d.target.LogicalBreakpoints, func(oldBp *proc.LogicalBreakpoint, err error) {
...
func (d *Debugger) state(retLoadCfg *proc.LoadConfig, withBreakpointInfo bool) (*api.DebuggerState, error) {
	...
	for _, thread := range d.target.ThreadList() {
		...
		if b := thread.Breakpoint(); b.Active && b.Breakpoint.Logical != nil {
			th.Breakpoint = d.convertBreakpoint(b.Breakpoint.Logical)
		}
	...
	for _, t := range d.target.ValidTargets() {
...
func (d *Debugger) CreateBreakpoint(requestedBp *api.Breakpoint, locExpr string, substitutePathRules [][2]string, suspended bool) (*api.Breakpoint, error) {
	...
	case len(requestedBp.Addrs) > 0:
		...
		if len(d.target.AllTargets()) == 1 {
			pid := d.target.Sel.Pid()
			for i, addr := range requestedBp.Addrs {
				addrs[i] = pidAddr{pid, addr}
			}
		} else {
			...
			for i, addr := range requestedBp.Addrs {
				addrs[i] = pidAddr{requestedBp.AddrPid[i], addr}
			}
		}
	default:
...
func (d *Debugger) convertBreakpoint(lbp *proc.LogicalBreakpoint) *api.Breakpoint {
	...
	tgtbps := d.findBreakpoint(lbp.LogicalID)
	pids := make([]int, len(tgtbps))
	bps := make([]*proc.Breakpoint, len(tgtbps))
	for i := range tgtbps {
		pids[i] = tgtbps[i].p.Pid()
		bps[i] = tgtbps[i].bp
	}
...
func (d *Debugger) amendBreakpoint(amend *api.Breakpoint) error {
	...
	if enabledBefore && !original.Enabled {
		if d.isWatchpoint(original) {
			return errors.New("can not disable watchpoints")
		}
		return d.target.DisableBreakpoint(original)
	}
	if !enabledBefore && original.Enabled {
		return d.target.EnableBreakpoint(original)
	}

	for _, bp := range d.findBreakpoint(amend.ID) {
		bp.bp.UserBreaklet().Cond = original.Cond
	}
	return nil
}

func (d *Debugger) isWatchpoint(bp *proc.LogicalBreakpoint) bool {
	bps := d.findBreakpoint(bp.LogicalID)
	if len(bps) == 0 {
		return false
	}
	return bps[0].bp.WatchType != 0
}
...
func (d *Debugger) ClearBreakpoint(requestedBp *api.Breakpoint) (*api.Breakpoint, error) {
	...
	if len(d.target.ValidTargets()) == 0 {
		_, err := d.target.AllTargets()[0].Valid()
		return nil, err
	}
	...
	if requestedBp.ID <= 0 {
		...
		bp := d.target.Sel.Breakpoints().M[requestedBp.Addr]
...
func (d *Debugger) Breakpoints(all bool) []*api.Breakpoint {
	...
	abps := []*api.Breakpoint{}
	if all {
		for _, p := range d.target.ValidTargets() {
			for _, bp := range p.Breakpoints().M {
				...
				api.ConvertPhysicalBreakpoints(abp, []int{p.Pid()}, []*proc.Breakpoint{bp})
...
func (d *Debugger) findBreakpoint(id int) []targetBreak {
	var bps []targetBreak
	for _, p := range d.target.ValidTargets() {
		for _, bp := range p.Breakpoints().M {
			if bp.LogicalID() == id {
				bps = append(bps, targetBreak{p, bp})
			}
		}
	}
	return bps
...
func (d *Debugger) Threads() ([]proc.Thread, error) {
	...
	return d.target.Sel.ThreadList(), nil
}
...
func (d *Debugger) FindThread(id int) (proc.Thread, error) {
	...
	for _, th := range d.target.Sel.ThreadList() {
		if th.ThreadID() == id {
			return th, nil
		}
	}
...
func (d *Debugger) FindGoroutine(id int) (*proc.G, error) {
	...
	return proc.FindGoroutine(d.target.Sel, id)
}
...
func (d *Debugger) Command(command *api.DebuggerCommand, resumeNotify chan struct{}) (*api.DebuggerState, error) {
	...
	case api.Call:
		...
		g := d.target.Sel.SelectedGoroutine()
		if command.GoroutineID > 0 {
			g, err = proc.FindGoroutine(d.target.Sel, command.GoroutineID)
			if err != nil {
				return nil, err
			}
		}
	...
	case api.SwitchThread:
		...
		for _, tgt := range d.target.ValidTargets() {
			if _, ok := tgt.FindThread(command.ThreadID); ok {
				d.target.Sel = tgt
				break
			}
		}
		err = d.target.Sel.SwitchThread(command.ThreadID)
	...
	case api.SwitchGoroutine:
		...
		g, err = proc.FindGoroutine(d.target.Sel, command.GoroutineID)
		if err == nil {
			err = d.target.Sel.SwitchGoroutine(g)
		}
		withBreakpointInfo = false
	case api.Halt:
...
func (d *Debugger) ThreadRegisters(threadID int, floatingPoint bool) (*op.DwarfRegisters, error) {
	...
	thread, found := d.target.Sel.FindThread(threadID)
	if !found {
	...
	return d.target.Sel.BinInfo().Arch.RegistersToDwarfRegisters(0, regs), nil
}
...
func (d *Debugger) ScopeRegisters(goid, frame, deferredCall int, floatingPoint bool) (*op.DwarfRegisters, error) {
	...
	s, err := proc.ConvertEvalScope(d.target.Sel, goid, frame, deferredCall)
	if err != nil {
		return nil, err
	}
	return &s.Regs, nil
...
func (d *Debugger) DwarfRegisterToString(i int, reg *op.DwarfRegister) (string, bool, string) {
	return d.target.Sel.BinInfo().Arch.DwarfRegisterToString(i, reg)
}
...
func (d *Debugger) Goroutines(start, count int) ([]*proc.G, int, error) {
	d.targetMutex.Lock()
	defer d.targetMutex.Unlock()
	return proc.GoroutinesInfo(d.target.Sel, start, count)
}
...
func (d *Debugger) FilterGoroutines(gs []*proc.G, filters []api.ListGoroutinesFilter) []*proc.G {
	...
	for _, g := range gs {
		ok := true
		for i := range filters {
			if !matchGoroutineFilter(d.target.Sel, g, &filters[i]) {
				ok = false
				break
			}
		}
...
func (d *Debugger) Stacktrace(goroutineID, depth int, opts api.StacktraceOptions) ([]proc.Stackframe, error) {
	...
	g, err := proc.FindGoroutine(d.target.Sel, goroutineID)
	if err != nil {
		return nil, err
	}

	if g == nil {
		return proc.ThreadStacktrace(d.target.Sel.CurrentThread(), depth)
	} else {
...
func (d *Debugger) Ancestors(goroutineID, numAncestors, depth int) ([]api.Ancestor, error) {
	...
	ancestors, err := proc.Ancestors(d.target.Sel, g, numAncestors)
	if err != nil {
		return nil, err
	}
...
func (d *Debugger) convertStacktrace(rawlocs []proc.Stackframe, cfg *proc.LoadConfig) ([]api.Stackframe, error) {
	...
	for i := range rawlocs {
		...
		if cfg != nil && rawlocs[i].Current.Fn != nil {
			var err error
			scope := proc.FrameToScope(d.target.Sel, d.target.Sel.Memory(), nil, rawlocs[i:]...)
...
func (d *Debugger) convertDefers(defers []*proc.Defer) []api.Defer {
	...
	for i := range defers {
		ddf, ddl, ddfn := defers[i].DeferredFunc(d.target.Sel)
		drf, drl, drfn := d.target.Sel.BinInfo().PCToLine(defers[i].DeferPC)
...
func (d *Debugger) CurrentPackage() (string, error) {
	...
	loc, err := d.target.Sel.CurrentThread().Location()
	if err != nil {
		return "", err
	}
	if loc.Fn == nil {
...
func (d *Debugger) Disassemble(goroutineID int, addr1, addr2 uint64) ([]proc.AsmInstruction, error) {
	...
	if addr2 == 0 {
		fn := d.target.Sel.BinInfo().PCToFunc(addr1)
		if fn == nil {
	...
	curthread := d.target.Sel.CurrentThread()
	if g != nil && g.Thread != nil {
		curthread = g.Thread
	}
	regs, _ := curthread.Registers()

	return proc.Disassemble(d.target.Sel.Memory(), regs, d.target.Sel.Breakpoints(), d.target.Sel.BinInfo(), addr1, addr2)
}

func (d *Debugger) AsmInstructionText(inst *proc.AsmInstruction, flavour proc.AssemblyFlavour) string {
	d.targetMutex.Lock()
	defer d.targetMutex.Unlock()
	return inst.Text(flavour, d.target.Sel.BinInfo())
}
...
func (d *Debugger) FindThreadReturnValues(id int, cfg proc.LoadConfig) ([]*proc.Variable, error) {
	...
	thread, found := d.target.Sel.FindThread(id)
	if !found {
...
func (d *Debugger) ListDynamicLibraries() []*proc.Image {
	d.targetMutex.Lock()
	defer d.targetMutex.Unlock()
	return d.target.Sel.BinInfo().Images[1:] // skips the first image because it's the executable file
...
func (d *Debugger) GetVersion(out *api.GetVersionOut) error {
	...
	if !d.isRecording() && !d.IsRunning() {
		out.TargetGoVersion = d.target.Sel.BinInfo().Producer()
	}
...
func (d *Debugger) ListPackagesBuildInfo(includeFiles bool) []*proc.PackageBuildInfo {
	d.targetMutex.Lock()
	defer d.targetMutex.Unlock()
	return d.target.Sel.BinInfo().ListPackagesBuildInfo(includeFiles)
}
...
func (d *Debugger) StopReason() proc.StopReason {
	d.targetMutex.Lock()
	defer d.targetMutex.Unlock()
	return d.target.Sel.StopReason
}
...
func (d *Debugger) DumpStart(dest string) error {
	...
	go func() {
		defer d.targetMutex.Unlock()
		d.target.Sel.Dump(fh, 0, &d.dumpState)
	}()
...
func (d *Debugger) BuildID() string {
	return d.target.Sel.BinInfo().BuildID
}

func (d *Debugger) GetBufferedTracepoints() []api.TracepointResult {
	traces := d.target.Sel.GetBufferedTracepoints()
	if traces == nil {
		return nil
	}
	results := make([]api.TracepointResult, len(traces))
	for i, trace := range traces {
		f, l, fn := d.target.Sel.BinInfo().PCToLine(uint64(trace.FnAddr))
