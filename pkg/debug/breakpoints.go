...
func createUnrecoveredPanicBreakpoint(t *Target) {
	panicpcs, err := proc.FindFunctionLocation(t, "runtime.startpanic", 0)
	if _, isFnNotFound := err.(*proc.ErrFunctionNotFound); isFnNotFound {
		panicpcs, err = proc.FindFunctionLocation(t, "runtime.fatalpanic", 0)
	}
	if err == nil {
...
func createFatalThrowBreakpoint(t *Target) {
	fatalpcs, err := proc.FindFunctionLocation(t, "runtime.fatalthrow", 0)
	if err == nil {
