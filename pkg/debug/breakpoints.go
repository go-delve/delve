...
func createUnrecoveredPanicBreakpoint(t *Target) {
	panicpcs, err := t.BinInfo().FindFunctionLocation(t, "runtime.startpanic", 0)
	if _, isFnNotFound := err.(*proc.ErrFunctionNotFound); isFnNotFound {
		panicpcs, err = t.BinInfo().FindFunctionLocation(t, "runtime.fatalpanic", 0)
	}
	if err == nil {
...
func createFatalThrowBreakpoint(t *Target) {
	fatalpcs, err := t.BinInfo().FindFunctionLocation(t, "runtime.fatalthrow", 0)
	if err == nil {
