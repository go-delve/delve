...
func (bp *Breakpoint) CheckCondition(thread proc.Thread, bi *BinaryInfo) BreakpointState {
...
func EvalBreakpointCondition(thread proc.Thread, bi *BinaryInfo, cond ast.Expr) (bool, error) {
	if cond == nil {
		return true, nil
	}
...
type WriteBreakpointFn func(addr uint64, instr []byte) (originalData []byte, err error)
...
func (bpmap *BreakpointMap) Set(loc *Location, kind BreakpointKind, instr []byte, cond ast.Expr, writeBreakpoint WriteBreakpointFn) (*Breakpoint, error) {
	if bp, ok := bpmap.M[loc.PC]; ok {
	...
	originalData, err := writeBreakpoint(loc.PC, instr)
	if err != nil {
		return nil, err
	}
	...
	bpmap.M[loc.PC] = newBreakpoint

	return newBreakpoint, nil
}

// SetWithID creates a breakpoint at addr, with the specified logical ID.
func (bpmap *BreakpointMap) SetWithID(id int, loc *Location, instr []byte, writeBreakpoint WriteBreakpointFn) (*Breakpoint, error) {
	bp, err := bpmap.Set(loc, UserBreakpoint, instr, nil, writeBreakpoint)
	if err == nil {
		bp.LogicalID = id
		bpmap.breakpointIDCounter--
	}
...
func (rbpi *returnBreakpointInfo) Collect(thread proc.Thread, bi *BinaryInfo) []*Variable {
	if rbpi == nil {
		return nil
	}
...
func returnInfoError(descr string, err error, mem proc.MemoryReadWriter) []*Variable {
...
func createUnrecoveredPanicBreakpoint(t *Target) {
	...
	if err == nil {
		loc := t.BinInfo().PCToLocation(panicpcs[0])
		bp, err := t.Breakpoints().SetWithID(unrecoveredPanicID, loc, t.BinInfo().Arch.BreakpointInstruction(), t.Process.WriteBreakpoint)
		if err == nil {
...
func createFatalThrowBreakpoint(t *Target) {
	...
	if err == nil {
		loc := t.BinInfo().PCToLocation(fatalpcs[0])
		bp, err := t.Breakpoints().SetWithID(fatalThrowID, loc, t.BinInfo().Arch.BreakpointInstruction(), t.Process.WriteBreakpoint)
		if err == nil {
			bp.Name = FatalThrow
		}
	}
