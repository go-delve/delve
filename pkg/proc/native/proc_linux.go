...
func Launch(cmd []string, wd string, flags proc.LaunchFlags, debugInfoDirs []string, tty string, redirects [3]string) (*proc.Target, error) {
	...
	_, _, err = wait(dbp, process.Process.Pid, 0)
	if err != nil {
...
func (dbp *nativeProcess) kill() error {
	...
	for {
		wpid, status, err := wait(dbp, dbp.pid, 0)
		if err != nil {
			return err
		}
...
func trapWait(cctx *proc.ContinueOnceContext, pid int) (*nativeThread, error) {
	return trapWaitInternal(cctx, pid, 0)
}
...
func trapWaitInternal(cctx *proc.ContinueOnceContext, pid int, options trapWaitOptions) (*nativeThread, error) {
	var waitdbp *nativeProcess = nil
	if cctx.NumProcs() == 1 {
		waitdbp = cctx.Proc(0).(*nativeProcess)
	}
	...
	for {
		...
		wpid, status, err := wait(waitdbp, pid, wopt)
		if err != nil {
		...
		if status.StopSignal() == sys.SIGTRAP && (status.TrapCause() == sys.PTRACE_EVENT_EXEC) {
			...
			dbp = newChildProcess(cctx.Proc(0).(*nativeProcess), wpid)
			tgt, err := dbp.initialize(findExecutable("", dbp.pid), cctx.DebugInfoDirs)
			if err != nil {
				_ = dbp.Detach(false)
				return nil, err
			}
...
func wait(dbp *nativeProcess, pid, options int) (int, *sys.WaitStatus, error) {
	var s sys.WaitStatus
...
func exitGuard(dbp *nativeProcess, cctx *proc.ContinueOnceContext, err error) error {
	if err != sys.ESRCH {
		return err
	}
	if status(dbp.pid, dbp.os.comm) == statusZombie {
		_, err := trapWaitInternal(cctx, -1, trapWaitDontCallExitGuard)
		return err
	}
...
func stop(cctx *proc.ContinueOnceContext, trapthread *nativeThread) (*nativeThread, error) {
	if numValid(cctx) == 0 {
		return nil, proc.ErrProcessExited{Pid: cctx.Proc(0).(*nativeProcess).pid}
	}
	...
	for {
		th, err := trapWaitInternal(cctx, -1, trapWaitNohang)
		if err != nil {
			return nil, exitGuard(procForThread(cctx, th.ID), cctx, err)
		}
		if th == nil {
			break
		}
		...
		for _, th := range dbp.threads {
			if th.os.running {
				if err := th.stop(); err != nil {
					return nil, exitGuard(dbp, cctx, err)
				}
			}
		}
	}
	...
	for {
		...
		_, err := trapWaitInternal(cctx, -1, trapWaitHalt)
		if err != nil {
			return nil, err
		}
	}
