...
func setStepIntoBreakpoint(dbp *Target, curfn *Function, text []AsmInstruction, cond ast.Expr) error {
	...
	// Resolve autogenerated wrappers before applying destination filters so the
	// filters inspect the function behind a call trampoline, not the trampoline.
...
func skipAutogeneratedWrappersIn(p Process, startfn *Function, startpc uint64, alwaysSkipFirst bool) (*Function, uint64) {
	...
	for range maxSkipAutogeneratedWrappers {
		...
		if fn.Trampoline {
			nextpc, ok := p.BinInfo().Arch.resolveCallTrampoline(p, fn, fn.Entry)
			if ok {
				startfn, startpc = p.BinInfo().PCToFunc(nextpc), nextpc
				if startfn == nil {
					return startfn, startpc
				}
				fn = startfn
