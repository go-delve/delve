...
func (grp *TargetGroup) StepOut() error {
	...
	if topframe.Ret != 0 {
		...
		bp, err := allowDuplicateBreakpoint(dbp.SetBreakpoint(0, retframe.Current.PC, NextBreakpoint, retFrameCond, 0))
		if err != nil {
			return err
		}
		if bp != nil {
	...
	if g != nil {
		if g.Thread == nil {
			...
			if _, err := dbp.SetBreakpoint(0, g.PC, NextBreakpoint,
				sameGoroutineCondition(dbp.BinInfo(), dbp.SelectedGoroutine(), thread.ThreadID()), 0); err != nil {
				return err
			}
			return grp.Continue()
		}
...
func next(dbp *Target, stepInto, inlinedStepOut bool) error {
	...
	for _, pc := range pcs {
		if _, err := allowDuplicateBreakpoint(dbp.SetBreakpoint(0, pc, NextBreakpoint, sameFrameCond, 0)); err != nil {
			dbp.ClearSteppingBreakpoints()
			return err
		}
	}
	...
	if !topframe.Inlined {
		...
		bp, _ := dbp.SetBreakpoint(0, retframe.Current.PC, NextBreakpoint, retFrameCond, 0)
...
func setStepIntoBreakpoints(dbp *Target, curfn *Function, text []AsmInstruction, topframe Stackframe, sameGCond ast.Expr) error {
	for _, instr := range text {
		...
		if instr.DestLoc != nil {
			...
			bp, err := allowDuplicateBreakpoint(dbp.SetBreakpoint(0, instr.Loc.PC, StepBreakpoint, sameGCond, 0))
			if err != nil {
				return err
			}
...
func setStepIntoBreakpointsReverse(dbp *Target, text []AsmInstruction, topframe Stackframe, sameGCond ast.Expr) error {
	...
	for i, instr := range text {
		...
		if nextIdx := i + 1; nextIdx < len(text) {
			...
			if !ok {
				if _, err := allowDuplicateBreakpoint(dbp.SetBreakpoint(0, text[nextIdx].Loc.PC, StepBreakpoint, sameGCond, 0)); err != nil {
					return err
				}
			}
		}
...
func setStepIntoBreakpoint(dbp *Target, curfn *Function, text []AsmInstruction, cond ast.Expr) error {
	...
	if _, err := allowDuplicateBreakpoint(dbp.SetBreakpoint(0, pc, NextBreakpoint, cond, 0)); err != nil {
		return err
	}
...
func setDeferBreakpoint(p *Target, text []AsmInstruction, topframe Stackframe, sameGCond ast.Expr, stepInto bool) (uint64, error) {
	...
	if deferpc != 0 && deferpc != topframe.Current.PC {
		bp, err := allowDuplicateBreakpoint(p.SetBreakpoint(0, deferpc, NextDeferBreakpoint, sameGCond, 0))
		if err != nil {
			return 0, err
		}
		if bp != nil && stepInto {
...
func stepOutReverse(p *Target, topframe, retframe Stackframe, sameGCond ast.Expr) error {
	...
	_, err = allowDuplicateBreakpoint(p.SetBreakpoint(0, callpc, NextBreakpoint, sameGCond, 0))
...
func (w *onNextGoroutineWalker) Visit(n ast.Node) ast.Visitor {
	if binx, isbin := n.(*ast.BinaryExpr); isbin && binx.Op == token.EQL {
		...
		if x == "runtime.curg.goid" || x == "runtime.threadid" {
			w.ret, w.err = evalBreakpointCondition(w.tgt, w.thread, n.(ast.Expr), 0)
			return nil
		}
	}
	return w
