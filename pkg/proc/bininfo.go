...
type BinaryInfo struct {
	...
	// key: function name, meta: *Function
	lookupRangeBodyFunc *trie.Trie
	...
	if fn.extraCache.rangeParent == nil {
		...
		for _, key := range lookupFunc.PrefixSearch(fn.Name) {
			if node, exist := lookupFunc.Find(key); exist {
				if fn2, ok := node.Meta().(*Function); ok && fn2.rangeParentName() == fn.Name {
...
func (bi *BinaryInfo) LookupRangeBodyFunc() *trie.Trie {
	if bi.lookupRangeBodyFunc == nil {
		bi.lookupRangeBodyFunc = trie.New()
		for i := range bi.Functions {
