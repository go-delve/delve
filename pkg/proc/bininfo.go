...
func loadBinaryInfoElf(bi *BinaryInfo, image *Image, path string, addr uint64, wg *sync.WaitGroup) error {
	...
	if err != nil {
		...
		if serr != nil {
			// We could not find debug information, fall back to pclntab.
			section := elfFile.Section(".gopclntab")
			if section == nil {
				return fmt.Errorf("could not read section .gopclntab from %s ", path)
			}
			tableData, err := section.Data()
			if err != nil {
				return fmt.Errorf("found section but could not read .gopclntab from %s ", path)
			}
